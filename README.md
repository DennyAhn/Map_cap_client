...
프로젝트 초기에는 ‘가장 짧은 길’이 아닌 **‘가장 안전한 길’** 제공을 1차 목표로 삼았습니다.  
하지만 문제의식과 잠재 사용자 요구를 분석하면서 **두 가지 축**으로 서비스를 고도화했습니다.

| 전략 | 세부 설명 |
|------|-----------|
| **① 사용자군 세분화 & 맞춤 정보** | - **일반** / **여성** / **노약자** / **장애인** 각 집단별로 가중치를 다르게 적용<br>  • 여성 → 비상벨·CCTV 밀집도 강조<br>  • 노약자 → 지하철 엘리베이터·경사로·휴게시설<br>  • 장애인 → 휠체어 충전소·무장애 화장실 등 |
| **② 지역 안전 개선 ‘참여형’ 기능** | - 가로등·보도블럭·맨홀 등 **시설물 파손/위험 구간 제보**<br> - 사진 + GPS 업로드 → 지자체·경찰 연계 처리 |

결국 **맞춤형 경로 안내 ＋ 참여형 안전 데이터 수집**으로,  
단순 길찾기 앱을 넘어 ***‘지역 안전 생태계’*** 를 구축합니다.
...
| 항목 | 내용 |
|------|------|
| **범위 집중** | 전국 단위 대신 **대구 → 계명대 인근**으로 특화 → 제한된 인력·비용으로도 임팩트 극대화 |
| **데이터 연계** | 공공데이터포털·경찰청·지자체 공개 데이터 **＋ 사용자 제보** → 신뢰성★ + 실시간성★ 확보 |
| **지속 가능성** | **지역 특화 MVP** 출시 → 데이터·사용자 pool을 기반으로 **대구 전역 → 타지역** 단계적 확장 |
...
1. **학생·주민 야간 이동 불안  ↓**  
2. **안전 취약계층(여성·노약자·장애인) 보호 ↑**  
3. 파손 시설물 **신속 보수 → 잠재 사고 예방 ＆ 지역 미관 개선**  
4. 안전 데이터 **시각화·공개 → 지역 공동체 참여 촉진**  
5. **소규모 개발팀**도 수행 가능한 현실적 프로젝트 모델 제시
## ✨ 핵심 기능
